...
<form name="Aqsc_Spare_Type_Add" action="Aqsc_Spare_Type.do" method="post" target="mFrame">
<div id="down_bg_2">
					...
					<table width="100%" border=1 cellPadding=0 cellSpacing=0 bordercolor="#3491D6" borderColorDark="#ffffff">		
						...
						<tr height='30'>
							...
							<td width='30%' align='left' colspan=3>
								<input type='text' name='Brand' style='width:96%;height:18px;' value='' maxlength='15'>
							</td>
...
function doAdd()
{
  if(Aqsc_Spare_Type_Add.CName.value.Trim().length < 1)
  {
    alert("请输入名称!");
    return;
  }
  if(Aqsc_Spare_Type_Add.Unit.value.Trim().length < 1)
  {
    alert("请输入单位!");
    return;
  }
  if(Aqsc_Spare_Type_Add.Model.value.Trim().length < 1)
  {
    alert("请输入型号!");
    return;
  }
  if(Aqsc_Spare_Type_Add.Brand.value.Trim().length < 1)
  {
    alert("请输入品牌!");
    return;
  }
  if(Aqsc_Spare_Type_Add.Des.value.Trim().length > 128)
  {
    alert("描述过长，请简化!");
    return;
  }
