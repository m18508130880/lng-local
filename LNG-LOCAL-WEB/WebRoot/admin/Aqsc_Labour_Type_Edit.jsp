...
<form name="Aqsc_Labour_Type_Edit" action="Aqsc_Labour_Type.do" method="post" target="mFrame">
<div id="down_bg_2">
...
function doEdit()
{
  if(Aqsc_Labour_Type_Edit.CName.value.Trim().length < 1)
  {
    alert("请输入名称!");
    return;
  }
  if(Aqsc_Labour_Type_Edit.Model.value.Trim().length < 1)
  {
    alert("请输入型号!");
    return;
  }
  if(Aqsc_Labour_Type_Edit.Unit.value.Trim().length < 1)
  {
    alert("请输入单位!");
    return;
  }
  if(Aqsc_Labour_Type_Edit.Brand.value.Trim().length < 1)
  {
    alert("请输入品牌!");
    return;
  }
  if(Aqsc_Labour_Type_Edit.Seller.value.Trim().length < 1)
  {
    alert("请输入供货单位!");
    return;
  }
  if(Aqsc_Labour_Type_Edit.Des.value.Trim().length > 128)
  {
    alert("描述过长，请简化!");
    return;
  }
