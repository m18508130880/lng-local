...
<%
  ...
  CurrStatus currStatus = (CurrStatus)session.getAttribute("CurrStatus_" + Sid);
	ArrayList Aqsc_Device_Breed = (ArrayList)session.getAttribute("Aqsc_Device_Breed_" + Sid);
	
%>
<body style="background:#CADFFF">
<form name="Aqsc_Device_Breed" action="Aqsc_Device_Breed.do" method="post" target="mFrame">
<div id="down_bg_2">
  <div id="cap"><img src="../skin/images/aqsc_device_breed.gif"></div><br><br><br>
				...
				<td width='100%' align='center' colspan=2>
					<table width="100%" style='margin:auto;' border=1 cellPadding=0 cellSpacing=0 bordercolor="#3491D6" borderColorDark="#ffffff">
			 			<tr>
							...
							<td width="20%" class="table_deep_blue">设备类型明细</td>
						</tr>
						<%
						if(Aqsc_Device_Breed != null)
						{
							int sn = 0;
							Iterator iterator = Aqsc_Device_Breed.iterator();
							while(iterator.hasNext())
							{
								AqscDeviceBreedBean statBean = (AqscDeviceBreedBean)iterator.next();
						...
						%>
								<tr <%=((sn%2)==0?"class='table_blue'":"class='table_white_l'")%>>
									...
									%>
									  ...
									  <td align=center><a href="#" title="点击查看" onClick="doDev('<%=Id%>', '<%=CName%>', '<%=Status%>')"><U>设备类型明细</U></a></td>
									<%
									}
									else
									{
									%>
									  ...
									  <td align=center><font color=gray><a href="#" title="点击查看" onClick="doDev('<%=Id%>', '<%=CName%>', '<%=Status%>')"><U>设备类型明细</U></a></font></td>
									<%
									}
									%>
								</tr>
...
function doAdd()
{
	location = "Aqsc_Device_Breed_Add.jsp?Sid=<%=Sid%>";
}

function doEdit(pId)
{
	location = "Aqsc_Device_Breed_Edit.jsp?Sid=<%=Sid%>&Id="+pId;
}

function doDev(pId, pCName, pStatus)
{
	var diag = new Dialog();
	diag.Top = "50%";
	diag.Width = 850;
	diag.Height = 450;
	diag.Title = "[设备品种: " + pCName + "]";
	diag.URL = "Aqsc_Device_Type.do?Cmd=0&Sid=<%=Sid%>&CType="+pId;
	diag.show();
}
</SCRIPT>
</html>