...
<form name="Spa_Store_O_Edt" action="Spa_Store_O.do" method="post" target="mFrame">
...
function doEdit()
{
	if(Spa_Store_O_Edt.Status_Memo.value.Trim().length < 1)
  {
  	alert('请填写作废原因!');
  	return;
  }
  if(Spa_Store_O_Edt.Status_Memo.value.Trim().length > 128)
  {
    alert("作废原因描述过长，请简化!");
    return;
  }
  if(confirm("确认审核?"))
  {
  	//Spa_Store_O_Edt.Cpm_Id.value = parent.window.parent.frames.lFrame.document.getElementById('id').value;
  	Spa_Store_O_Edt.submit();
  }
}
</SCRIPT>
</html>