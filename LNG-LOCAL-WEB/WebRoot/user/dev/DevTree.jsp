...
<body style="background:#0B80CC;">
	<div><ul id="devTree" class="ztree"></ul></div>
	<div id='CurrJsp' style='display:none'>Dev_List.jsp</div>
...
/*-----------------------------生成树----------------------------------*/
$('#devTree').empty();
$.fn.zTree.init($('#devTree'), setting, Nodes1);

/*-----------------------------点击树----------------------------------*/
function zTreeOnClick(event, treeId, treeNode)
{
	if(document.getElementById('CurrJsp').innerText != 'Dev_List.jsp' 
	&& document.getElementById('CurrJsp').innerText != 'Dev_Remind.jsp')
	{
		return;
	}
	if(0 == treeNode.level)
	{
		...
		if(subrootValue.length > 0)
		{
			...
			for(var i=0; i<list.length; i++)
			{
				...
				{
					var zTree_Dev = $.fn.zTree.getZTreeObj('devTree');
...
function afterLoad()
{
	...
	if('<Limit:limitValidate userrole='<%=FpList%>' fpid='1001' ctype='1'/>' == '')
	{//设备明细
		window.parent.frames.mFrame.location = "Dev_List.do?Cmd=0&Sid=<%=Sid%>&Cpm_Id="+document.getElementById('id').value+"&Func_Corp_Id=9999&Func_Sel_Id=9";
	}
	else
	{
		if('<Limit:limitValidate userrole='<%=FpList%>' fpid='1002' ctype='1'/>' == '')
		{//持证提示
			window.parent.frames.mFrame.location = "Dev_Remind.do?Cmd=0&Sid=<%=Sid%>&Cpm_Id="+document.getElementById('id').value+"&Func_Corp_Id=9999&Func_Sel_Id=0";
		}
	}
}
afterLoad();
