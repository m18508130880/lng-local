...
<body style="background:#0B80CC;">
	<div><ul id="fixTree" class="ztree"></ul></div>
	<div id='CurrJsp' style='display:none'>Fix_Trace.jsp</div>
...
/*-----------------------------生成树----------------------------------*/
$('#fixTree').empty();
$.fn.zTree.init($('#fixTree'), setting, Nodes1);

/*-----------------------------点击树----------------------------------*/
function zTreeOnClick(event, treeId, treeNode)
{
	if(document.getElementById('CurrJsp').innerText != 'Fix_Trace.jsp' 
	&& document.getElementById('CurrJsp').innerText != 'Fix_Ledger.jsp')
	{
		return;
	}
	if(0 == treeNode.level)
	{
		...
		if(subrootValue.length > 0)
		{
			...
			for(var i=0; i<list.length; i++)
			{
				...
				{
					var zTree_Dev = $.fn.zTree.getZTreeObj('fixTree');
...
function afterLoad()
{
	...
	if('<Limit:limitValidate userrole='<%=FpList%>' fpid='1201' ctype='1'/>' == '')
	{//故障跟踪
		window.parent.frames.mFrame.location = "Fix_Trace.do?Cmd=0&Sid=<%=Sid%>&Cpm_Id="+document.getElementById('id').value+"&Func_Corp_Id=9999&Func_Sub_Id=0";
	}
	else
	{
		if('<Limit:limitValidate userrole='<%=FpList%>' fpid='1202' ctype='1'/>' == '')
		{//故障统计
  		...
  		var Month = BTime.substring(5,7);
			window.parent.frames.mFrame.location = "Fix_Ledger.do?Cmd=0&Sid=<%=Sid%>&Cpm_Id="+document.getElementById('id').value+"&Func_Corp_Id=9999&Func_Sel_Id=1&BTime="+BTime+"&ETime="+ETime+"&Year="+Year+"&Month="+Month;
		}
	}
}
afterLoad();
