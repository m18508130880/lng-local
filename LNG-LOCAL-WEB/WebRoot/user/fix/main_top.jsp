...
<body>
	<div class="top">
		...
		<div class="bar">
			...
			<div id="banner_r">
				<ul>
					<li style="display:<Limit:limitValidate userrole='<%=FpList%>' fpid='1201' ctype='1'/>">
						<img onClick="doFix01()" style="cursor:hand" src="../../skin/images/top_menu_fix_01.gif" alt="故障跟踪"/>
					</li>	
					<li style="display:<Limit:limitValidate userrole='<%=FpList%>' fpid='1202' ctype='1'/>">
						<img onClick="doFix02()" style="cursor:hand" src="../../skin/images/top_menu_fix_02.gif" alt="故障统计"/>
					</li>
				</ul>
			</div>
		</div>
...
function doFix01()
{
	window.parent.frames.mFrame.location = "Fix_Trace.do?Cmd=0&Sid=<%=Sid%>&Cpm_Id="+window.parent.frames.lFrame.document.getElementById('id').value+"&Func_Corp_Id=9999&Func_Sub_Id=0";
}
...
function doFix02()
{
  ...
  var Month = BTime.substring(5,7);
	window.parent.frames.mFrame.location = "Fix_Ledger.do?Cmd=0&Sid=<%=Sid%>&Cpm_Id="+window.parent.frames.lFrame.document.getElementById('id').value+"&Func_Corp_Id=9999&Func_Sel_Id=1&BTime="+BTime+"&ETime="+ETime+"&Year="+Year+"&Month="+Month;
}
</SCRIPT>
</html>