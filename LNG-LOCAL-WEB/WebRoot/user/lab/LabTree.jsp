...
<body style="background:#0B80CC;">
	<div><ul id="labTree" class="ztree"></ul></div>
	<div id='CurrJsp' style='display:none'>Lab_Store.jsp</div>
...
/*-----------------------------生成树----------------------------------*/
$('#labTree').empty();
$.fn.zTree.init($('#labTree'), setting, Nodes1);

/*-----------------------------点击树----------------------------------*/
function zTreeOnClick(event, treeId, treeNode)
{
	if(document.getElementById('CurrJsp').innerText != 'Lab_Store.jsp' 
	&& document.getElementById('CurrJsp').innerText != 'Lab_Store_I.jsp' 
	&& document.getElementById('CurrJsp').innerText != 'Lab_Store_IN.jsp' 
	&& document.getElementById('CurrJsp').innerText != 'Lab_Store_O.jsp')
	{
		return;
	}
	if(0 == treeNode.level)
	{
		...
		if(subrootValue.length > 0)
		{
			...
			for(var i=0; i<list.length; i++)
			{
				...
				{
					var zTree_Dev = $.fn.zTree.getZTreeObj('labTree');
...
function afterLoad()
{
	...
	if('<Limit:limitValidate userrole='<%=FpList%>' fpid='0901' ctype='1'/>' == '')
	{//库存台账
		window.parent.frames.mFrame.location = "Lab_Store.do?Cmd=0&Sid=<%=Sid%>&Cpm_Id="+document.getElementById('id').value+"&Func_Corp_Id=9999";
	}
	else
	{
		if('<Limit:limitValidate userrole='<%=FpList%>' fpid='0902' ctype='1'/>' == '')
		{//申购记录
			window.parent.frames.mFrame.location = "Lab_Store_I.do?Cmd=0&Sid=<%=Sid%>&Cpm_Id="+document.getElementById('id').value+"&Func_Corp_Id=9999&Func_Sub_Id=0&Func_Type_Id=";
		}
		else
		{
			if('<Limit:limitValidate userrole='<%=FpList%>' fpid='0903' ctype='1'/>' == '')
			{//入库记录
				window.parent.frames.mFrame.location = "Lab_Store_I.do?Cmd=1&Sid=<%=Sid%>&Cpm_Id="+document.getElementById('id').value+"&Func_Corp_Id=9999&Func_Sub_Id=0&Func_Type_Id=";
			}
			else
			{
				if('<Limit:limitValidate userrole='<%=FpList%>' fpid='0904' ctype='1'/>' == '')
				{//出库记录
					window.parent.frames.mFrame.location = "Lab_Store_O.do?Cmd=0&Sid=<%=Sid%>&Cpm_Id="+document.getElementById('id').value+"&Func_Corp_Id=9999&Func_Sub_Id=9";
				}
			}
		}
	}
