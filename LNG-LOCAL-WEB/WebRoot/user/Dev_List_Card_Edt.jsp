...
<%
  ...
  String FpId   = UserInfo.getFp_Role();
	...
	String Card_BTime = "";
	String Card_ETime = "";
	String Card_All_BTime = "";
	String Card_All_ETime = "";
	String Card_Review = "";
	String Card_Overall = "";
	String Overall_Inteval = "";
	if(Dev_List_Card_List != null)
	{
		Iterator iterator = Dev_List_Card_List.iterator();
		while(iterator.hasNext())
		{
			DevListCardBean Bean = (DevListCardBean)iterator.next();
			if(Bean.getSN().equals(SN) && Bean.getCard_Type().equals(Card_Type))
		  {
		  	Card_Type_Name = Bean.getCard_Type_Name();
		  	Card_Id = Bean.getCard_Id();
		  	Card_BTime = Bean.getCard_BTime();
		  	Card_ETime = Bean.getCard_ETime();
		  	Card_All_BTime = Bean.getCard_All_BTime();
		  	Card_All_ETime = Bean.getCard_All_ETime();
		  	Card_Review = Bean.getCard_Review();
		  	Card_Overall = Bean.getCard_Overall();
		  	Overall_Inteval = Bean.getOverall_Inteval();
		  }
		}
	}
...
<form name="Dev_Lis_Cardt_Edt" action="Dev_List_Card.do" method="post" target="mFrame">
<table width="100%" border=1 cellPadding=0 cellSpacing=0 bordercolor="#3491D6" borderColorDark="#ffffff">		
	...
	<tr height='40' id='Overall'>		
		...
		<td width='30%' align='left'>
		  <input name='Card_All_BTime' type='text' style='width:96%;height:18px;' value='<%=Card_All_BTime%>' onClick='WdatePicker({readOnly:true})' class='Wdate' maxlength='10'>
		</td>
		<td width='20%' align='center'>全面复检年限</td>
		<td width='30%' align='left'>
			<select name='Card_Overall' style='width:98%;height:20px;'>
				<option value='1' <%=Card_Overall.equals("1")?"selected":""%>>1年</option>
				<option value='2' <%=Card_Overall.equals("2")?"selected":""%>>2年</option>
				<option value='3' <%=Card_Overall.equals("3")?"selected":""%>>3年</option>
				<option value='4' <%=Card_Overall.equals("4")?"selected":""%>>4年</option>
				<option value='5' <%=Card_Overall.equals("5")?"selected":""%>>5年</option>
				<option value='6' <%=Card_Overall.equals("6")?"selected":""%>>6年</option>
				<option value='7' <%=Card_Overall.equals("7")?"selected":""%>>7年</option>
				<option value='8' <%=Card_Overall.equals("8")?"selected":""%>>8年</option>
			</select>
		</td>
	</tr>
	<tr height='40'>
...
function doAdd()
{
	if('<Limit:limitValidate userrole='<%=FpList%>' fpid='130107' ctype='1'/>' == 'none')
	{
		alert('您无权限修改证件信息!');
		return;
	}
  if(Dev_Lis_Cardt_Edt.Card_Id.value.Trim().length < 1)
  {
    alert("请填写证件编号!");
    return;
  }
  if(Dev_Lis_Cardt_Edt.Card_BTime.value.length < 1)
  {
    alert("请选择检定日期!");
    return;
  }
  var Card_ETime = (parseInt(Dev_Lis_Cardt_Edt.Card_BTime.value.substring(0,4)) + parseInt(Dev_Lis_Cardt_Edt.Card_Review.value)) + Dev_Lis_Cardt_Edt.Card_BTime.value.substring(4,10);
  var Card_All_BTime = '';
  var Card_All_ETime = '';
  var Card_Overall = 0;
  if(document.getElementById('Overall').style.display == '')
  {
  	Card_All_BTime = Dev_Lis_Cardt_Edt.Card_All_BTime.value;
  	Card_All_ETime = (parseInt(Dev_Lis_Cardt_Edt.Card_All_BTime.value.substring(0,4)) + parseInt(Dev_Lis_Cardt_Edt.Card_Overall.value)) + Dev_Lis_Cardt_Edt.Card_All_BTime.value.substring(4,10);
  	Card_Overall = Dev_Lis_Cardt_Edt.Card_Overall.value;
  }
  else
  {
  ...
  if(confirm("信息无误,确定提交?"))
  {
  	if(window.XMLHttpRequest)
	  {
		...
		var url = 'Dev_List_Card.do?Cmd=11&Sid=<%=Sid%>&SN=<%=SN%>&Card_Type=<%=Card_Type%>&Operator=<%=Operator%>'
		        + '&Card_Id='+Dev_Lis_Cardt_Edt.Card_Id.value
		        + '&Card_BTime='+Dev_Lis_Cardt_Edt.Card_BTime.value
		        + '&Card_ETime='+Card_ETime
		        + '&Card_Review='+Dev_Lis_Cardt_Edt.Card_Review.value
