...
<%
	...
	if(Cad_Action != null)
	{
		...
		while(iterator.hasNext())
		{
			...
			{
				Observe_OP = Bean.getObserve_OP();
				Observe_BY = Bean.getObserve_BY();
				Observe_Time = Bean.getObserve_Time();
				Observe_Addr = Bean.getObserve_Addr();
				Observe_Act = Bean.getObserve_Act();
				Observe_Des = Bean.getObserve_Des();
				Act_Plan = Bean.getAct_Plan();
				Act_Sugg = Bean.getAct_Sugg();
				Status = Bean.getStatus();
				if(null == Observe_Act){Observe_Act = "";}
				if(null == Observe_Des){Observe_Des = "";}
				if(null == Act_Plan){Act_Plan = "";}
				if(null == Act_Sugg){Act_Sugg = "";}
			}
		}
	}
...
<form name="Cad_Action_Edt" action="Cad_Action.do" method="post" target="mFrame">
<div id="down_bg_2">
	<div id="cap"><img src="../skin/images/cad_action_edt.gif"></div><br><br><br>
				...
				<td width='100%' align='center'>
					<table width="100%" border=1 cellPadding=0 cellSpacing=0 bordercolor="#3491D6" borderColorDark="#ffffff">
										...
										while(useriter.hasNext())
										{
											...
											if(userBean.getStatus().equals("0"))
											{
												switch(userBean.getDept_Id().length())
												{
													case 2:
									%>
															<option value='<%=userBean.getSys_Id()%>' <%=userBean.getSys_Id().equals(Observe_OP)?"selected":""%>><%=userBean.getCName()%></option>
									<%
														break;
													case 10:
														String Role_List = "";
										...
										while(useriter.hasNext())
										{
											...
											if(userBean.getStatus().equals("0"))
											{
												switch(userBean.getDept_Id().length())
												{
													case 2:
									%>
															<option value='<%=userBean.getSys_Id()%>' <%=userBean.getSys_Id().equals(Observe_BY)?"selected":""%>><%=userBean.getCName()%></option>
									<%
														break;
													case 10:
														String Role_List = "";
						...
						<tr height='30'>
							...
							<td width='75%' align='left'>
								<input type='text' name='Observe_Time' style='width:118px;height:18px;' value='<%=Observe_Time%>' onClick='WdatePicker({readOnly:true})' class='Wdate' maxlength='10'>
							</td>
						</tr>
						<tr height='30'>
						...
						<tr height='30'>
							...
							<td width='75%' align='left'>
								<input type='text' name='Observe_Addr' style='width:98%;height:18px;' value='<%=Observe_Addr%>' maxlength=64>
							</td>
						</tr>
						<tr height='30'>
								...
								<table width="100%" border=1 cellPadding=0 cellSpacing=0 bordercolor="#3491D6" borderColorDark="#ffffff">
									...
									%>
												<tr height=30>
													...
													<td width=45% align=center>全都安全<input type='checkbox' id='checkbox<%=cnt%>' name='checkbox<%=cnt%>' value='<%=typeBean.getId()%>' <%=CommUtil.isContain(Observe_Act, typeBean.getId())?"checked":""%> onClick="doSeltAll(this.value, '<%=cnt%>')"></td>
												</tr>
									<%
												cnt++;
												dnt++;
									...
									%>
													<tr height=30>
														...
														<td width=45% align=left><input type='checkbox' id='checkbox<%=cnt%>' name='checkbox<%=cnt%>' value='<%=typeBean.getId()%>' <%=CommUtil.isContain(Observe_Act, typeBean.getId())?"checked":""%>><%=typeBean.getCName()%></td>
														<td width=45% align=left rowspan='<%=curr_index%>'><textarea id='textarea<%=dnt%>' name='textarea<%=dnt%>' rows='10' cols='30' maxlength=128><%=Observe_Des.split("\\^")[dnt-1]%></textarea></td>
													</tr>
									<%
												}
												else
						...
						<tr height='30'>
							...
							<td width='75%' align='left'>
								<textarea name='Act_Plan' rows='5' cols='53' maxlength=128><%=Act_Plan%></textarea>
							</td>
						</tr>
						<tr height='30'>
							<td width='25%' align='center'>整改建议</td>
							<td width='75%' align='left'>
								<textarea name='Act_Sugg' rows='5' cols='53' maxlength=128><%=Act_Sugg%></textarea>
							</td>
						</tr>
						<tr height='30'>
...
//window.parent.frames.lFrame.document.getElementById('CurrJsp').innerText = 'Cad_Action_Edt.jsp';
...
function doAdd()
{
  if(Cad_Action_Edt.Observe_OP.value.length < 1 || Cad_Action_Edt.Observe_OP.value == '*')
  {
  	alert('请选择观察人!');
  	return;
  }
  if(Cad_Action_Edt.Observe_BY.value.length < 1 || Cad_Action_Edt.Observe_BY.value == '*')
  {
  	alert('请选择被观察人!');
  	return;
  }
  if(Cad_Action_Edt.Observe_Time.value.length < 1)
  {
  	alert('请选择观察时间!');
  	return;
  }
  if(Cad_Action_Edt.Observe_Addr.value.Trim().length < 1)
  {
  	alert('请填写观察地点!');
  	return;
  }
  ...
  Cad_Action_Edt.Observe_Act.value = Observe_Act;
  ...
  Cad_Action_Edt.Observe_Des.value = Observe_Des;
  
  if(confirm("信息无误，确认提交?"))
  {
  	Cad_Action_Edt.UId.value = Cad_Action_Edt.Observe_BY.value;
  	Cad_Action_Edt.submit();
  }
}
</SCRIPT>
</html>