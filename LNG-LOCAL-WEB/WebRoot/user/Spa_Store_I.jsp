		...
		<tr height='25px' class='sjtop'>
			<td width='80%' align='left'>
				...
				<select name='Func_Sel_Id' style='width:70px;height:20px' onChange="doSelect()">
					...
					<option value='0' <%=currStatus.getFunc_Sel_Id() == 0 ? "selected":""%>>待审核</option>
					<option value='1' <%=currStatus.getFunc_Sel_Id() == 1 ? "selected":""%>>审核有效</option>
					<option value='2' <%=currStatus.getFunc_Sel_Id() == 2 ? "selected":""%>>审核无效</option>
				</select>
				...
				申购单号:<input type='text' name='Func_Type_Id' style='width:70px;height:16px;' value='<%=currStatus.getFunc_Type_Id()%>'>
			</td>
			...
			<td width='100%' align='center' colspan=2>
				<table width="100%" border=1 cellPadding=0 cellSpacing=0 bordercolor="#3491D6" borderColorDark="#ffffff">
					<tr height='25'>
						...
						<td width='8%' align='center' class="table_deep_blue">备品备件</td>
						<td width='18%' align='center' class="table_deep_blue">规格型号</td>
						<td width='8%' align='center' class="table_deep_blue">申购时间</td>
						<td width='37%' align='center' class="table_deep_blue">申购信息</td>
						<td width='24%' align='center' class="table_deep_blue">审核信息</td>
					</tr>
					<%
					if(Spa_Store_I != null)
					{
						...
						while(iterator.hasNext())
						{
							...
							String str_Des1    =  "数量: <font color=red>" + Spa_I_Cnt+"个</font>"
							                   + "&nbsp;&nbsp;&nbsp;"
							                   + "单价: " + Spa_I_Price+"元"
							                   + "&nbsp;&nbsp;&nbsp;"
							                   + "金额: " + Spa_I_Amt+"元"
					...
					%>
						  <tr <%=((sn%2)==0?"class='table_blue'":"class='table_white_l'")%>>
						    ...
						    <td align='center' style='cursor:hand;' title='编辑申购信息' onclick="doEdit01('<%=SN%>', '<%=Status%>')"><%=str_Des1%></td>
						    <td align='center' style='cursor:hand;' title='审核申购信息' onclick="doEdit02('<%=SN%>', '<%=Status%>')"><%=str_Des2%></td>
							</tr>
					<%
						}
					}
