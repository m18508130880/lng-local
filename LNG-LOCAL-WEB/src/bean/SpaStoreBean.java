	...
	public void ExecCmd(HttpServletRequest request, HttpServletResponse response, Rmi pRmi, boolean pFromZone) throws ServletException, IOException
	{
		...
		switch(currStatus.getCmd())
		{
			...
			case 0://查询
		    	...
		    	//备品类型
		    	AqscSpareTypeBean SpareType = new AqscSpareTypeBean();
		    	SpareType.setFunc_Sub_Id("");
		    	msgBean = pRmi.RmiExec(0, SpareType, 0);
		    	request.getSession().setAttribute("Spa_Store_Type_" + Sid, ((Object)msgBean.getMsg()));
		    	
		    	//站点库存
		    	SpaStationBean Station = new SpaStationBean();
		    	Station.setFunc_Corp_Id(Func_Corp_Id);
		    	Station.setFunc_Sub_Id(Func_Sub_Id);
		    	msgBean = pRmi.RmiExec(0, Station, 0);
	...
	public void ExportToExcel(HttpServletRequest request, HttpServletResponse response, Rmi pRmi, boolean pFromZone) 
	{
		try
		{
			...
			if(null != temp0)
			{
	            ...
	            {
	            	label = new Label(7+a, D_Index, "");
		            sheet.addCell(label);
	            }
	            sheet.mergeCells(0,D_Index,7+dev_cnt,D_Index);
	            ...
	            for(int a=1; a<=dev_cnt; a++)
	            {
	            	if(1 == a)
	            	{
	            		label = new Label(7+a, D_Index, "各站分配情况", wff2);
	            		sheet.addCell(label);
	            	}
	            	else
	            	{
	            		label = new Label(7+a, D_Index, "", wff2);
	            		sheet.addCell(label);
	            	}
	            }
	            if(dev_cnt > 0)
	            	sheet.mergeCells(8,D_Index,7+dev_cnt,D_Index);
			    ...
			    if(Role_List.length() > 0 && null != User_Device_Detail)
				{
					...
					while(iterator.hasNext())
					{
						...
						{
							dev_dex++;
							label = new Label(7+dev_dex, D_Index, statBean.getBrief(), wff2);
				            sheet.addCell(label);
						}
					}
				}			    
		            ...
		            for(int a=0; a<dev_cnt; a++)
					{
						...
						if(null != temp1)
						{
							...
							while(statiter.hasNext())
							{
								...
								{
									stat_Des = "备用: " + stationBean.getSpa_I_Cnt()
									 		 + "\012"
									 		 + "在用: " + stationBean.getSpa_O_Cnt()
									 		 + "\012"
									 		 + "报废: " + stationBean.getSpa_S_Cnt()
									 		 + "\012"
									 		 + "合计: " + (Integer.parseInt(stationBean.getSpa_I_Cnt()) + Integer.parseInt(stationBean.getSpa_O_Cnt()) + Integer.parseInt(stationBean.getSpa_S_Cnt()));
								}
							}
						}
						label = new Label(8+a, D_Index, stat_Des, wff3);
			            sheet.addCell(label);
					}
				}
	...
	*/
	...
	public String getSql(int pCmd)
	{
		...
		switch (pCmd)
		{
			case 0://查询
				...
				break;
			case 1://调整查询
				Sql = " select t.spa_type, t.spa_type_name, t.spa_mode, t.spa_i_cnt, t.spa_o_cnt, t.spa_s_cnt, t.spa_r_cnt, t.ctype, t.model, t.unit, t.brand, t.seller, t.ctime, t.operator, t.operator_name, t.memo " +
				  	  " from view_spa_store_log t " +
				  	  " where t.spa_type like '"+ Func_Corp_Id +"%' " +
				  	  "   and t.ctype like '"+ Func_Sub_Id +"%' " +
			...
			case 2://全部查询-检定维修
				...
				break;
			case 10://添加
				Sql = " insert into spa_store(spa_type, spa_mode, spa_r_cnt, ctime, operator)values('"+ Spa_Type +"', '"+ Spa_Mode +"', '"+ Spa_R_Cnt +"', DATE_FORMAT(now(), '%Y-%m-%d %H:%i:%S'), '"+ Operator +"')";
				break;
			case 11://保底
				Sql = " update spa_store t set t.spa_r_cnt = '"+ Spa_R_Cnt +"' where t.spa_type = '"+ Spa_Type +"' and t.spa_mode = '"+ Spa_Mode +"' ";
				break;
			case 13://维修队实时库存调整
				Sql = " update spa_store t set t.spa_i_cnt = '"+ Spa_I_Cnt +"', t.spa_o_cnt = '"+ Spa_O_Cnt +"', t.spa_s_cnt = '"+ Spa_S_Cnt +"' where t.spa_type = '"+ Spa_Type +"' and t.spa_mode = '"+ Spa_Mode +"' ";
				break;
			case 14://维修队实时库存调整日志
				Sql = " insert into spa_store_log(spa_type, spa_mode, spa_i_cnt, spa_o_cnt, spa_s_cnt, memo, ctime, operator)" +
					  " values('"+ Spa_Type +"', '"+ Spa_Mode +"', '"+ Spa_I_Cnt +"', '"+ Spa_O_Cnt +"', '"+ Spa_S_Cnt +"', '"+ Memo +"', DATE_FORMAT(now(), '%Y-%m-%d %H:%i:%S'), '"+ Operator +"')";
				break;
			case 20://报废
	...
	public boolean getData(ResultSet pRs)
	{
		...
		try
		{
			...
			setSpa_I_Cnt(pRs.getString(4));
			setSpa_O_Cnt(pRs.getString(5));
			setSpa_S_Cnt(pRs.getString(6));
			setSpa_R_Cnt(pRs.getString(7));
			setCType(pRs.getString(8));
			setModel(pRs.getString(9));
			setUnit(pRs.getString(10));
			setBrand(pRs.getString(11));
			setSeller(pRs.getString(12));
